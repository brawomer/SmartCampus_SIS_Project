...
export default function TeacherDashboard() {
  ...
  const handleCourseClick = (courseId: number) => {
    ...
    fetch(`https://localhost:7229/api/Teacher/course-students/${courseId}`)
      .then(res => {
        if (!res.ok) {
          throw new Error(`API Error ${res.status}: ${res.statusText}. The server returned an error. Check backend logs.`);
        }
        return res.json();
      })
      .then((data: Student[]) => {
      ...
      .catch(err => {
        ...
        setError(err.message || "Failed to load students. Check backend server.");
        setLoading(false);
      });
  };
      ...
      <main className="flex-1 ml-6 space-y-6">
        <header className="flex justify-between items-center text-white">
          <div>
            ...
            <p className="text-white/60 text-sm">Graduation Project</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {error && (
            <div className="col-span-full bg-red-500/20 border border-red-500/50 rounded-xl p-4 text-white text-sm flex items-center space-x-2">
          ...
          {courses.length === 0 ? (
            <div className="col-span-full text-white/60 text-center py-8">
              No courses assigned. Please check your account.
            </div>
          ) : (
            courses.map((course) => (
              <div 
        ...
        {selectedCourse && (
          <div className="backdrop-blur-xl bg-white/10 rounded-3xl border border-white/20 p-8 shadow-2xl animate-in fade-in slide-in-from-bottom-4">
            ...
            {loading && <div className="text-white/60 text-center py-8">Loading students...</div>}

            {!loading && students.length === 0 && (
              <div className="text-white/60 text-center py-8">No students found for this course.</div>
            )}
